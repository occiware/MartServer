...
import org.occiware.mart.servlet.exception.EntityConflictException;
import org.occiware.mart.servlet.exception.ResponseParseException;
import org.occiware.mart.servlet.facade.AbstractPutQuery;
...
public class PutQuery extends AbstractPutQuery {
    ...
    public Response inputQuery(@PathParam("path") String path, @Context HttpHeaders headers, @Context HttpServletRequest request) {
        ...
        for (Data data : datas) {
            ...
            PathParser pathParser = new PathParser(data, path, super.getRequestParameters());

            if (pathParser.isInterfQuery()) {
                try {
                    response = ((OCCIServletOutputParser) occiResponse).parseResponse("you cannot use interface query on PUT method", Response.Status.BAD_REQUEST);
                    return response;
     ...
     */
    @Override
    public Response defineMixinTag(final Data data, final String path) {
